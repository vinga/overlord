...
Restarts the **server** (port 3173) and optionally the **client** (port 5173).
...
**IMPORTANT:** Only kill processes bound to ports 3173/5173 — do NOT kill all node processes, as that destroys active Claude sessions.
...
## Health check (use this to verify, not `lsof -i :3173`)

**Gotcha:** `lsof -i :3173` prints the port as `hbci` (its IANA service name) on macOS, so grepping for `:3173` in the output misses it. Always pass `-P` (disables port-name resolution) or just hit the HTTP endpoint.

```bash
# Reliable: HTTP probe — returns 200 when server is healthy
curl -sS -o /dev/null -w "server:%{http_code}\n" http://localhost:3173/api/info

# Reliable: numeric port listing
lsof -nP -iTCP:3173 -sTCP:LISTEN
lsof -nP -iTCP:5173 -sTCP:LISTEN
```

Treat `server:200` as the single source of truth. Bridge `ENOENT` spam in the log from stale pipes is normal and does NOT mean the server failed to start — check for `Overlord server listening on http://localhost:3173` in the log instead.
...
for i in $(seq 1 150); do
  code=$(curl -sS -o /dev/null -w "%{http_code}" --max-time 1 http://localhost:3173/api/info 2>/dev/null)
  [ "$code" = "200" ] && break
  sleep 0.1
done
...
```bash
# Kill ONLY processes on ports 3173 and 5173 (not all node processes!)
powershell -Command "
  @(3173, 5173) | ForEach-Object {
    \$port = \$_
...
sleep 4
echo "Server: http://localhost:3173  |  Client: http://localhost:5173"
